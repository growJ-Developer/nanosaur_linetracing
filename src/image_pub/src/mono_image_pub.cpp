...
class MonoImagePublisher : public rclcpp::Node
{
    public:
        ...
        : Node("mono_image_publisher")
        {
            ...
            image_pub_ = this->create_publisher<sensor_msgs::msg::Image>(namespace_ + "/simple_rover/camera/image_raw", 2);
            image_info_pub_ = this->create_publisher<sensor_msgs::msg::CameraInfo>(namespace_ + "/simple_rover/camera/camera_info", 2);
            ...
            std::string cmd2 = "60/1 ! nvvidconv flip-method=2 ! video/x-raw, width=640, height=480, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink";
